...
bool Ctrl::open(uint32_t fbnum,
   ...
   if(!getScreenInfo(mInfo)) {
      LOGE("Ctrl failed to getScreenInfo");
      return false;
   }
...
bool Ctrl::start(const utils::PipeArgs& args)
{
   ...
   if (args.whf.format & INTERLACE_MASK &&
       (args.whf.format != HAL_PIXEL_FORMAT_YV12)) {
   ...
   if (fmt < 0) {
      LOGE("Ctrl failed getMdpFormat unsopported "
   ...
   const utils::PipeArgs newargs(flags, // mdp flags
                                 ...
                                 args.reconf);
   if (!setInfo(newargs)) {
      LOGE("Ctrl failed to setInfo mdpflags=%d wait=%d zorder=%d",
   ...
   if(!mRot->start(newargs)) {
      LOGE("Ctrl failed to start Rotation session");
      return false;
   }
   ...
   if(!mRot->remap(utils::ROT_NUM_BUFS, newargs)) {
      LOGE("%s Error in remapping", __FUNCTION__);
   }

   if(!mMdp.set()) {
      LOGE("Ctrl start failed set overlay");
      return false;
   }
...
{
   // In reconf there are alot of stuff different.
   // FIXME: We should be able to merge it to one
   // updateSource func in the future.
   if(utils::RECONFIG_ON == args.reconf) {
      // args format is still HAL, not MDP fmt.
      return mMdp.setSourceReconf(mRot, args, mInfo);
   }
   ...
   if(mOvBufInfo != whf) {
      ...
      mMdp.setUserData(0);
      if(!mRot->start(newargs)) {
         LOGE("%s failed start rot", __FUNCTION__);
         return false;
      }
...
bool Ctrl::setPosition(const utils::Dim& dim)
{
   if(!dim.check(mInfo.mFBWidth, mInfo.mFBHeight)) {
      LOGE("Ctrl setPosition error in dim");
      dim.dump();
      return false;
   }

   if(!mMdp.setPosition(dim, mInfo.mFBWidth, mInfo.mFBHeight)) {
      LOGE("Ctrl failed MDP setPosition");
      return false;
   }
   return true;
}
...
bool Ctrl::setParameter(const utils::Params& p)
{
   ...
   case utils::OVERLAY_DITHER:
      // nothing here today
      LOGE("Ctrl setParameter OVERLAY_DITHER not impl");
      return true;
   case utils::OVERLAY_TRANSFORM:
      if(!mRot->overlayTransform(mMdp, trns)) {
         LOGE("Ctrl setParameter failed Rot overlayTransform");
         return false;
      }
      break;
   default:
      LOGE("Ctrl setParameter unknown param %d", p.param);
      return false;
   }
   return true;
}
...
bool Ctrl::setCrop(const utils::Dim& d)
{
   ...
   if(!mMdp.setCrop(d)) {
      LOGE("Data setCrop failed in MDP setCrop");
      return false;
   }
   mCrop = d;
   return true;
...
void Ctrl::dump() const {
   LOGE("== Dump Ctrl start ==");
   LOGE("orient=%d", mOrient);
   mInfo.dump("mInfo");
   mMdp.dump();
   mRot->dump();
   LOGE("== Dump Ctrl end ==");
